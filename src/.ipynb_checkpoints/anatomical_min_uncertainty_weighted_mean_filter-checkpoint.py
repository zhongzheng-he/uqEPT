...
def anatomical_min_uncertainty_weighted_mean_filter(Im,Ref, uncertainty, kernel_size=[21,21,21], shape="cube", thresh=0.05, ROI=None, n_jobs=-1):
    ...
    def process_patch(j):
        ...
        Shape_patch &= Shape
        # Avoid padded/background voxels.
        ...
        # Remove inf uncertainties before selection.
        valid = (np.isfinite(uncertainty_patch_in_shape))
        uncertainty_patch_in_shape = uncertainty_patch_in_shape[valid]
        # if all uncertainties are inf, return median value
        if uncertainty_patch_in_shape.size == 0:
            return np.nanmedian(Im_patch_in_shape)
        Im_patch_in_shape = Im_patch_in_shape[valid]
    
        # Select the lowest-uncertainty quarter of valid pairs.
        n_selected = max(Im_patch_in_shape.size // 4, 1)
        idx_sorted = np.argsort(uncertainty_patch_in_shape)[:n_selected]

        # if there is only one selected voxel, return the lowest-uncertainty value
        if len(idx_sorted) == 1:
