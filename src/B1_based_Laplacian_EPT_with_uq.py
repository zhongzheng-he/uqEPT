...
def B1_based_Laplacian_EPT_with_uq(B, Ref, kernel_size=[11,11,11], shape="cube", thresh=0.05, omega=128e6*2*np.pi, h=None, ROI=None, n_jobs=-1):
    ...
    def process_patch(j):
        ...
        if np.array_equal(Shape_patch, Shape): 
            ...
            C0 = F0_pinv @ B_patch_in_shape # coefficients matrix 
            # F_adap=F0   
            return calculate_EPs_and_uncertainties(F0, C0, F0_pinv, B_patch_in_shape,j_mu0_omega, omega_eps0, J_c, J_biv)
        else:
            ...
            F_adap = F[ind, :]
            F_pinv = pinv(F_adap)
            C = F_pinv @ B_patch_in_shape# coefficients matrix
            return calculate_EPs_and_uncertainties(F_adap, C, F_pinv, B_patch_in_shape,j_mu0_omega, omega_eps0, J_c, J_biv)
