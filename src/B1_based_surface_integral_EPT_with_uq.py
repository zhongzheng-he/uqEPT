...
def B1_based_surface_integral_EPT_with_uq(B,Ref,fit_kernel_size=[11,11, 11],int_kernel_size=[15, 15, 15],fit_shape = "cube",int_shape = "cube",
    ...
    def process_fit_patch(j):
        ...
        # avoid if the center value of Ref_patch is close to zero value (background)
    ...
    def process_integral_patch(j):
        ...
        # Old-equivalent Stage-2 component extraction. If the center is False,
        # keep_center_component returns an all-False mask; the local SI helper
        # then returns ok=False because V is exactly zero.
