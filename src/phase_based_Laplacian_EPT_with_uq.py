...
def phase_based_Laplacian_EPT_with_uq(PhiTR, Ref, kernel_size=[11,11,11], shape="cube", thresh=0.05, omega=128e6*2*np.pi, h=None, ROI=None, n_jobs=-1):
    ...
    def process_patch(j):
        ...
        if np.array_equal(Shape_patch, Shape): 
            ...
            C0 = F0_pinv @ PhiTR_patch_in_shape # coefficients matrix 
            # F_adap=F0   
            return  calculate_conductivity_and_uncertainty(F0, C0, F0_pinv, PhiTR_patch_in_shape, J,coeff)
            ...
            F_adap = F[ind, :] 
            F_pinv= pinv(F_adap)
            C=  F_pinv @ PhiTR_patch_in_shape     
            return  calculate_conductivity_and_uncertainty(F_adap, C, F_pinv, PhiTR_patch_in_shape, J,coeff)
...
@njit(cache=True)
def calculate_conductivity_and_uncertainty(F_adap, C, F_pinv, PhiTR_patch_in_shape, J,coeff):
    ...
    if dof <= 0 or np.linalg.matrix_rank(F_adap) < 10: # rank verification
        return sigma, np.inf
    ...
    M = F_pinv @ F_pinv.T # For full rank F:  inv(F.T@F) = F_pinv @ F_pinv.T
