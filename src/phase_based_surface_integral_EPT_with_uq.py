...
def phase_based_surface_integral_EPT_with_uq(
    PhiTR,
    Ref,
    ...
    """
    Phase-only surface-integral EPT with Level-2 uncertainty quantification.

    This version follows the same implementation style as the refined complex-B
    surface-integral method:

        - padded input arrays for fitting and integration;
        - direct padded-index assignment for output maps;
        - adaptive anatomical kernels based on Ref;
        - 6-connected center-component extraction;
        - local 3x3 convolution-at-voxel for SI weights;
        - Numba-accelerated covariance propagation.

    Conductivity is estimated from the transceive phase as

        sigma_SI = S_phi / (2 * mu0 * omega * V_eff)
    ...
    """
    print("Phase SI level-2 UQ refined")
    start_time = time.time()

    # ------------------------------------------------------------------------------------------
    # Input verification
    # ------------------------------------------------------------------------------------------
    if not isinstance(PhiTR, np.ndarray):
        raise ValueError("PhiTR must be a numpy array.")
    ...
    def process_integral_patch(j):
        ...
        return calculate_surface_integral_phase_and_uncertainty(
            q_patch,
            cov_q_patch,
            Shape_patch,
            valid_fit_patch,
...
@njit(cache=True)
def calculate_surface_integral_phase_and_uncertainty(
    q_patch,
    cov_q_patch,
    Shape_patch,
    valid_fit_patch,
...
def imrescale(image, new_min=0, new_max=1, ROI=None, window=(0.005, 0.995)):
    """Intensity normalization with optional ROI and percentile clipping."""
